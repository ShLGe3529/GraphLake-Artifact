...
CALL {
  ...
  MATCH (person1:Person)<-[:HAS_CREATOR]-(message1:Comment)-[:HAS_TAG]->(tag:Tag {name: paramTagX})
  WHERE date(message1.creation_date) = date(paramDateX)
  // filter out Persons with more than $maxKnowsLimit friends who created the same kind of Message
  OPTIONAL MATCH (person1)-[:KNOWS]-(person2:Person)<-[:HAS_CREATOR]-(message2:Comment)-[:HAS_TAG]->(tag)
  WHERE date(message2.creation_date) = date(paramDateX)
