...
| **GraphLake** | — | First `gsql` schema load: GPE `Build EdgeRefBlocks takes Xms` → seconds | REST BI-16 |
...
|--------|---------------|-------------|
| **GraphLake** | First `gsql` schema load: GPE `Build EdgeRefBlocks takes Xms` → seconds (same as exp1) | REST per BI query |
...
| `GRAPHLAKE_IMAGE` | `shlge3529/graphlake-artifact:latest` | exp1/exp2: `docker load` + `docker run` |
